...
func GetUsers() gin.HandlerFunc {
	return func(c *gin.Context){
		...
		//Ideally want to return all the users base on the various query parameter
	}
}
...
func SignUp() gin.HandlerFunc{
	return func(c *gin.Context){
		...
		if err != nil{
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while checking fro the email"})
			return
		}
		//Hash Password
		...
		//generate token and refresh token (generate all tokens from helper)
